...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		//// Switch statment on type of BTreePageId ////
		switch(pid.pgcateg()){
			//// If LEAF, return getpage with input arguments ////
			case BTreePageId.LEAF:
				return(BTreeLeafPage)(this.getPage(tid, dirtypages, pid, perm)); // maybe change how cast

			//// If INTERNAL, move to the left-most page with READ_ONLY permissions ////
			...
			//// Other Two Cases where we do nothing ////
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		//// Add an empty page for the right sibling //// Move the left half of the tuples over to make room
        BTreeLeafPage rightSiblingPage = (BTreeLeafPage)(this.getEmptyPage(tid, dirtypages, BTreePageId.LEAF));

		int count = 0;
        double tuplesToMove = ceil(page.getMaxTuples() / 2);
        Iterator<Tuple> tupleIterator = page.reverseIterator();

		Tuple currentTuple = createPageRight(page, rightSiblingPage, count, tuplesToMove, tupleIterator);

		//// Re-assign the sibling pointers of affected leaf pages ////
		rightSiblingPage.setRightSiblingId(page.getRightSiblingId());
        if(page.getRightSiblingId() != null){
        	...
        	previous_rightChild.setLeftSiblingId(rightSiblingPage.getId());
		}

		page.setRightSiblingId(rightSiblingPage.getId());
        rightSiblingPage.setLeftSiblingId(page.getId());
        ...
        BTreeEntry newEntry = new BTreeEntry(keyField, page.getId(), rightSiblingPage.getId());
        parentPage.insertEntry(newEntry);

        //// Update Parent pointers ////
        this.updateParentPointers(tid, dirtypages, parentPage);

        //// Use <= to find which page to insert the new tuple in ////
	...
	//// Helper function for splitLeafPage and stealFromLeafPage ////
	// Creates a page on the right and moves half the tuples from the BTreeLeafPage 'page' over
	private Tuple createPageRight(BTreeLeafPage page, BTreeLeafPage rightSiblingPage, int count, double tuplesToMove, Iterator<Tuple> tupleIterator) throws DbException {
		Tuple currentTuple = null;
		while((count < tuplesToMove) && tupleIterator.hasNext()){
            currentTuple = tupleIterator.next();
            page.deleteTuple(currentTuple);
            rightSiblingPage.insertTuple(currentTuple);
            count++;
        }
		return currentTuple;
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		BTreeEntry middleEntry = tupleIterator.next();
		Field fieldKey = middleEntry.getKey();
		BTreeInternalPage parentPage = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		page.deleteKeyAndRightChild(middleEntry);
		middleEntry.setLeftChild(page.getId());
		middleEntry.setRightChild(newPage.getId());
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int count = 0;
        double tuplesToMove = (sibling.getNumTuples() - page.getNumTuples()) / 2;
		...
		Tuple currentTuple = createPageRight(sibling, page, count, tuplesToMove, tupleIterator);

		if(isRightSibling){
            currentTuple = tupleIterator.next();
        }

        entry.setKey(currentTuple.getField(this.keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        BTreeEntry currentEntry = parentEntry;
        BTreeEntry rightEntry = page.iterator().next();

        for (int i = 0; i < entriesToMove; i++) {
            BTreeEntry leftEntry = entryIterator.next();

            parent.deleteKeyAndRightChild(currentEntry);
            currentEntry.setLeftChild(leftEntry.getRightChild());
            currentEntry.setRightChild(rightEntry.getLeftChild());
            page.insertEntry(currentEntry);

            leftSibling.deleteKeyAndRightChild(leftEntry);
            leftEntry.setLeftChild(leftSibling.getId());
            leftEntry.setRightChild(page.getId());
            parent.insertEntry(leftEntry);

            rightEntry = currentEntry;
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        for (int i = 0; i < entriesToMove; i++) {
            BTreeEntry rightEntry = entryIterator.next();
            ...
            rightEntry.setLeftChild(page.getId());
            rightEntry.setRightChild(rightSibling.getId());
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//// Move all Tuples from Right Page to Left Page ////
		...
		//// Updating the sibling pointers ////
		leftPage.setRightSiblingId(rightPage.getRightSiblingId());
		if(rightPage.getRightSiblingId() != null){
			BTreeLeafPage rightSiblingPage = (BTreeLeafPage)(this.getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE));
		...
		//// Set the Right Page as an Empty Page ////
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		//// Push Parent down and move pointers ////
		this.deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);

		BTreeEntry rightParentSibling = rightPage.iterator().next();
		BTreeEntry leftParentSibling = leftPage.reverseIterator().next();

		parentEntry.setLeftChild(leftParentSibling.getRightChild());
		parentEntry.setRightChild(rightParentSibling.getLeftChild());
		...
		BTreeEntry entryBTree = null;

		while(entryIterator.hasNext()){
			entryBTree = entryIterator.next();
			rightPage.deleteKeyAndRightChild(entryBTree);
			leftPage.insertEntry(entryBTree);
		}

		//// Update Parent Pointers so that children of leftPage correctly point to parent ////
